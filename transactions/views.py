...
class PurchaseCreateView(View):                                                 
    ...
    def post(self, request, pk):
        formset = PurchaseItemFormset(request.POST)                             # receives a post for the formset
        supplierobj = get_object_or_404(Supplier, pk=pk)                        # gets the supplier object

        if formset.is_valid():
            # create PurchaseBill directly (minimal change, no new imports)
            try:
            ...
            try:
                # create purchase bill details object (use PurchaseBillDetails)
            ...
            for item_form in formset:                                             # save each individual form as its own object
                billitem = item_form.save(commit=False)
                billitem.billno = billobj                                         # link the bill object to the items
                ...
                stock.quantity += billitem.quantity                               # update stock quantity
        ...
        # if formset invalid -> re-render with empty formset & supplier
